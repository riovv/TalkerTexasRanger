 ...
 */
talker.plugin.loadAll = function () {
  talker.conf.plugins.forEach(function (plugin) {
    try {
      talker.plugin.load(plugin);
...
talker.connect = function (rooms, token) {
  ...
  rooms.forEach(function (room) {
    ...
    talker.room.connected[room.name] = r;
    r.connect();
  });
};
...
['broadcast', 'command', 'connect', 'log', 'message', 'on', 'plugin'].forEach(function (method) {
