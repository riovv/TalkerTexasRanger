...
talker.plugins.load = function (name) {
  ...
  try{
    ...
    loadedPlugin = require(path);  
  }catch(e){
    // Maybe it's a folder?
    // Try ../plugins/name/name
    var folderPath = '../plugins/' + name + '/' + name;

    // If exception here, package cannot be found.
    loadedPlugin = require(folderPath);
  }

  // Successfully loaded ../plugins/name
  return loadedPlugin;
};
...
talker.plugins.loadAll = function () {
  talker.conf.plugins.forEach(function (plugin) {
    try{
      ...
      // Success
      talker.log("Loaded plugin : '" + plugin.white + "' ");  
    }catch(e){
      if (e.message.substr(0, 18) !== 'Cannot find module') {
        // Error loading plugin
        // Continue loading TalkerTexasRanger
        talker.log("!!Warning!!: ".rainbow.bold  + " Cannot find plugin '" + plugin.magenta + "' ");  
      }else{
        // Some other error, halt
        throw e;
      }
    }
...
talker.command = function (command, callback, rooms) {
  ...
  var listener = function (data) {
    ...
    if (new RegExp(pattern).test(data.content)) {
